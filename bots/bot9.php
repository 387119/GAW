...
function get_strategy_types($vStrategies){
	// на вход получаем массив стратегий, на выход отдаёт уникальный список типов под которые есть стратегии, который будем использовать для выборки из базы аккаунтов только этих типов
	$iTypes=array();
	foreach ($vStrategies as $iStrategy){
		if (array_key_exists("acc_types",$iStrategy)){
			$iTypes=array_merge($iTypes,$iStrategy["acc_types"]);
		}
	}
	$iTypes=array_unique($iTypes,SORT_REGULAR);
	return $iTypes;
}
function get_bst(){
	global $gaw;
...
$gStrategyTypes=get_strategy_types($gStrategies);
print_r($gStrategyTypes);
die();
// START
// менеям принцип работы, надо в цикле постоянно получать список командиров в которые надо зайти и проверить что там твоится и достроить если потребуется, 1 итерация цикла, 1 делаем,
// выбираем список всех акков которые надо брать, а потом пробегаемся по этому списку в обратном порядке, если же условия для какогото командира выполнены, то просто пропускаем его и ничего с ним не делаем.
// таким образом мы будем пропускать акки на которых все условия постройки выполнены, но мы будем их пробегать, это конечно время ненужное на выполнение, но так как это просто сравнение данных с базой, то это секунды, можно пренебречь, зато в случае модификации акка, или стратегии, или типов, бот автоматически подхватит весь набор аккаунтов, без необходимости делать дополнительные правки в базе
$gaw=new GAW();
//$gaw->dbQuery("select * from users where");
